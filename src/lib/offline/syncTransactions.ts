...
export async function syncTransactionsToServer() {
  ...
  console.log(
    `Tentative de synchronisation de ${pending.length} transaction(s) en attente`,
  );
  ...
  for (const tx of pending) {
    try {
      ...
      if (tx.productId && (tx.type === "sale" || tx.type === "credit")) {
        ...
        if (fetchErr || !product) {
          console.warn(
            `Impossible de récupérer le stock pour la transaction ${tx.localId}`,
            fetchErr,
          );
          skippedCount++;
          continue;
        ...
        if (updateErr) {
          console.error(
            `Échec de la mise à jour du stock (transaction ${tx.localId})`,
            updateErr,
          );
          failedCount++;
          continue;
        ...
        console.log(
          `Stock mis à jour pour ${product.name} → -${tx.quantity} (nouveau: ${newStock})`,
        );
      }
      ...
      console.log(
        `Transaction ${tx.localId} synchronisée avec succès (ID serveur: ${data.transactionId})`,
      );
    } catch (err) {
      console.error(
        `Échec de la synchronisation de la transaction ${tx.localId}`,
        err,
      );
      failedCount++;
